...
//---------------------------------- ACK 0x01 ----------------------------------//
TEST(ATE401, ACK) {
  std::vector<uint8_t> arr = { '#', '@', '!', sizeof(state) + 2, static_cast<uint8_t>(Pi401Cmd::ACK)};
  ...
  arr.insert(arr.begin() + MAGIC.size(), arr.size() - 3 + 2); //  3 - magic, 2 len/crc
  arr.push_back(calculateCRC8(arr.data(), arr.size()));

  uint8_t* result = getPackStart(arr.data(), arr.size(), MAGIC);
  ASSERT_NE(result, nullptr);
}
...
//---------------------------------- TXD 0x04 ----------------------------------//
TEST(ATE401, TXD) {
  uint8_t arr[] = { '#', '@', '!', 3, static_cast<uint8_t>(Pi401Cmd::TXD), 0 };
...
//---------------------------------- RTE 0x07 ----------------------------------//
TEST(ATE401, RTE) {
  uint8_t arr[] = { '#', '@', '!', 3, static_cast<uint8_t>(Pi401Cmd::RTE), 0 };
...
//---------------------------------- DC 0x08 ----------------------------------//
TEST(ATE401, DC) {
  uint8_t arr[] = { '#', '@', '!', 3, static_cast<uint8_t>(Pi401Cmd::DC), 0 };
...
//---------------------------------- TMP 0x0A ----------------------------------//
TEST(ATE401, TMP) {
  uint8_t arr[] = { '#', '@', '!', 3, static_cast<uint8_t>(Pi401Cmd::TMP), 0 };
...
//---------------------------------- BUTTON 0x0B ----------------------------------//
TEST(ATE401, BUTTON) {
  uint8_t arr[] = { '#', '@', '!', 3, static_cast<uint8_t>(Pi401Cmd::BUTTON), 0 };
...
TEST(ATE401, WIFI_CRED) {
  ...
  uint8_t arr[] = { '#', '@', '!', 11, static_cast<uint8_t>(Pi401Cmd::WIFI_CRED), 'I', 'T', 'V', NTS, 'P', 'S', 'W', 'D', NTS };
